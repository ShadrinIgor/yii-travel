...
class ExtMinScript extends CClientScript {
	...
	public function minScriptCreateGroup($files) {
		$files = (array)array_diff( $files, $this->includedFiles );
		// Prevent old scripts from loading again
		...
		return $this->minScriptBaseUrl . Yii::app()->createUrl(
			$this->minScriptControllerId . '/serve',
			$params
		);
	}
