...
class WorkController extends Controller
{
    public function actionIndex( $inputMessage = "" )
    {
        ...
        {
            Yii::app()->page->title = "Мои вакансии";
            $message = "";

            if( empty( $message ) && !empty( $inputMessage ) )$message = $inputMessage;

            $items = CatalogWork::fetchAll( DBQueryParamsClass::CreateParams()->setConditions("user_id=:user_id AND is_resume=0")->setParams( array( ":user_id"=>Yii::app()->user->id ) )->setOrderBy("id DESC")->setLimit(-1)->setCache(0) );
    ...
    public function actionDescription()
    {
        if( !Yii::app()->user->isGuest )
        {
            ...
            $id = (int)Yii::app()->request->getParam("id", 0);
            if( !empty( $id ) )$item = CatalogWorkVacansyAdd::fetch( $id );
                          else $item = new CatalogWorkVacansyAdd();
            ...
            if( !empty( $_POST["update"] ) )
            {
                ...
                $item->setAttributesFromArray( $_POST["CatalogWorkVacansyAdd"] );
                $item->is_resume = 0;
                ...
                if( $item->save() )
                {
                    if( !$isAdd )$message = "Описание успешно обновленно";
                            else $message = "Вакансия успешно добавленна";
                }
