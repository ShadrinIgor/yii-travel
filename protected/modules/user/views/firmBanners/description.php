                ...
                <td>
                    <?php if( !$checkedRequest ) : ?>
                        <?php if( $count<$maxCount ) : ?>
                            ...
                            <span>
                                <?= Yii::t("user", "Для размещения бесплатного главного баннера сайта ( находящийся в верхней части сайта ) , необходимо заполнить все поля затем поставить галочку и уже после этого нажать СОХРАНИТЬ."); ?><br/>
                                <?= Yii::t("user", "После получения заявки наш менеджер рассмотрит Ваш баннер, и если он соответствует правилам сайта то сразу разместит сроком на 10 дней."); ?>
                            </span>
                            <hr/>
                        <?php endif; ?>
                        <?php if( $count == 0 ) : ?>
                            <div class="messageSummaryGreen"><?= Yii::t("user", "Еще не подано не одной заявки Ваш баннер может быть <b>ПЕРВЫМ</b> и пока <b>единственным</b>, торопитесь"); ?>....</div>
                        <?php elseif( $count<$maxCount ) : ?>
                            <?= Yii::t("user", "Получено заявок"); ?>: <b><?= $count ?></b><br/>
                        ...
                        <?php else : ?>
                            ...
                            <div class="errorSummary"><?= Yii::t("user", "К сожалению все заявки уже заняты, и сейчас Вы не можете разместить Свой баннер, через 10 дней будут приниматься новые заявки, не <b>УПУСТИТЕ МОМЕНТ</b>"); ?> </div>
                        <?php endif; ?>
                    <?php else: ?>
                            <div class="messageSummaryGreen"><?= Yii::t("user", "Ваша заявка успешно принята и передана на рассмотрение менеджеру, и если Ваш баннер соответствует правилам сайта то он сразу разместит его сроком на 10 дней. После размещение Вашего баннера менеджер пришлет Вам уведомление на "); ?><b><?= $firm->email ?></b>.<br/><?= Yii::t("user", "Если после подачи заявки пришло 2 или более дней,а баннер не размещен и нет уведомления, напишите в службу поддержки <b><a href='mailto: support@world-travel.uz'> support@world-travel.uz</a></b>, и мы обязательно проконтролируем."); ?></div>
                    <?php endif; ?>
                </td>
            </tr>
            <tr>
