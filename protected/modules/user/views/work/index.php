...
<?php
    $this->widget('addressLineWidget', array(
        'links'=>array( Yii::t("user", "Мои вакансии" ) ),
    ));
?>
<h1><?= Yii::t("user", "Мои вакансии") ?></h1>
<?php if( $message ) :?>
...
<?php foreach( $items as $item ):
    ...
    <tr <?= $item->is_hot==1 ? 'class="isHot"' : "" ?>>
        ...
        <td>
            <a href="<?= SiteHelper::createUrl( "/user/work/description", array("id"=>$item->id) ) ?>" title="<?= Yii::t("user", "описание") ?>"><?= $item->name ?></a><br/>
        ...
        <td>
            ...
            <div class="itemAction textAlignCenter">
                ...
                <a href="<?= SiteHelper::createUrl("/user/work/description", array("id"=>$item->id)) ?>"><?= Yii::t("page", "Описание") ?></a><br/>
                <div class="popup PMarginLeft">
                    <br/>
                    <b><?= Yii::t("user", "Вы действительно хотите удалить запись?") ?></b>
                    <br/><br/>
                    <a href="#" class="PCancel"><?= Yii::t("user", "Отмена") ?> </a>&nbsp;|&nbsp;
                    <a href="<?= SiteHelper::createUrl("/user/work/delete", array("id"=>$item->id)) ?>"><?= Yii::t("user", "Удалить") ?></a>
                </div>
...
<?php endforeach; ?>
    ...
    <div class="textAlignCenter"><br/><a href="<?= SiteHelper::createUrl( "/user/work/description" ) ?>">[ <?= Yii::t("user", "Добавить вакансию"); ?> ]</a></div>
</div>