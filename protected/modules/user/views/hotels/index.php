...
<?php
    $this->widget('addressLineWidget', array(
        'links'=>array( Yii::t("user", "Мои отели" ) ),
    ));
?>
<h1><?= Yii::t("user", "Мои отели" ) ?></h1>
...
<?php foreach( $items as $item ):
    ...
    <tr <?= $item->is_hot==1 ? 'class="isHot"' : "" ?>>
        ...
        <td>
            <a href="<?= SiteHelper::createUrl( "/user/hotels/description", array("id"=>$item->id) ) ?>" title="<?= Yii::t("user", "описание") ?>"><?= $item->name ?></a>
        </td>
        ...
        <td class="textAlignCenter">
            ...
            <div class="itemAction textAlignCenter">
                <a href="<?= SiteHelper::createUrl("/user/hotels/description", array("id"=>$item->id)) ?>"><?= Yii::t("page", "Описание") ?></a><br/>
                <div>
                ...
                <div class="popup PMarginLeft">
                    ...
                    <a href="<?= SiteHelper::createUrl("/user/hotels/delete", array("id"=>$item->id)) ?>"><?= Yii::t("user", "Удалить") ?></a>
                </div>
...
<?php endforeach; ?>
    ...
    <br/>
    <center>
        <a href="<?= SiteHelper::createUrl("/user/hotels/description") ?>"><?= Yii::t("user", "Добавить отель" ) ?></a>
    </center>
</div>