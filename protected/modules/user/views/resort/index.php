...
?>
<h1><?= Yii::t("user", "Мои курорты/зоны отдыха/дачи") ?></h1>
...
<?php foreach( $items as $item ):
    ...
    <tr <?= $item->is_hot==1 ? 'class="isHot"' : "" ?>>
        ...
        <td>
            <a href="<?= SiteHelper::createUrl( "/user/resort/description", array("id"=>$item->id) ) ?>" title="<?= Yii::t("user", "описание") ?>"><?= $item->name ?></a>
        </td>
        ...
        <td class="textAlignCenter">
            ...
            <div class="itemAction textAlignCenter">
                <a href="<?= SiteHelper::createUrl("/user/resort/description", array("id"=>$item->id)) ?>"><?= Yii::t("page", "Описание") ?></a><br/>
                <div>
                    <?php if( $item->active == 1 ) : ?>
                        <a href="#" class="publishLink" onclick="return ajaxAction( this, '<?= SiteHelper::createUrl("/user/firms/setPublish", array("id"=>$item->id, "catalog"=>"catalogKurorts")) ?>', '' );"><?= Yii::t("user", "Снять с публикации") ?></a><br/>
                    <?php else : ?>
                        <a href="#" class="publishLink"  onclick="return ajaxAction( this, '<?= SiteHelper::createUrl("/user/firms/setPublish", array("id"=>$item->id, "catalog"=>"catalogKurorts")) ?>', '' );"><?= Yii::t("user", "Опубликовать") ?></a><br/>
                    <?php endif; ?>
                </div>
                ...
                <div class="popup PMarginLeft">
                    ...
                    <a href="<?= SiteHelper::createUrl("/user/resort/delete", array("id"=>$item->id)) ?>"><?= Yii::t("user", "Удалить") ?></a>
                </div>
...
<?php endforeach; ?>
    ...
    <br/>
    <center>
        <a href="<?= SiteHelper::createUrl("/user/resort/description") ?>"><?= Yii::t("user", "Добавить запись" ) ?></a>
    </center>
</div>