<?php foreach( $items as $item ) :
    ?>
    <div class="listItems">
        <?php if( $item->image ) : ?><div class="IImage"><a title="<?= SiteHelper::getTranslateForUrl( $item->name ) ?> ?>" href="<?= SiteHelper::createUrl( $url."/description", array(  "id"=>$item->id, "slug"=>SiteHelper::getSlug($item) )) ?>"><img src="<?= ImageHelper::getImage( $item->image, 2 ) ?>" width="200" alt="<?= $item->name ?>" /></div><?php endif; ?>
        <div class="LHeader">
        ...
        <div class="LParams">
            ...
            <a href="<?= SiteHelper::createUrl("resorts/", array( "category_id"=>$item->category_id->id, "slug"=>SiteHelper::getSlug($item->category_id) )) ?>" title="курорты <?= $item->category_id->name ?>"><b><?= $item->category_id->name ?></b></a><br/>
