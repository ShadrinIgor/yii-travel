<?php foreach( $items as $item ) :
    ...
    <div class="listItems">
        ...
        <div class="LParams">
            ...
            <?= Yii::t("page", "Страна"); ?>: <a href="<?= SiteHelper::createUrl("/hotels/country")."/".$item->country_id->slug  ?>.html" title="<?= $item->country_id->name ?>"><b><?= $item->country_id->name ?></b></a><br/>
            <?= Yii::t("page", "Город"); ?>: <a href="<?= SiteHelper::createUrl("/hotels/city")."/".$item->city_id->slug  ?>.html" title="<?= $item->city_id->name ?>"><b><?= $item->city_id->name ?></b></a><br/>
        </div>
