<div class="IBItem">
    ...
    <div class="LParams">
        <a href="<?= SiteHelper::createUrl("resorts/index", array("country_id"=>$item->country_id->id, "slug"=>SiteHelper::getSlug( $item->country_id ))) ?>" title="зоны отдаха <?= $item->country_id->name_2 ?>"><?= $item->country_id->name ?></b></a><br/>
        <a href="<?= SiteHelper::createUrl("resorts/index", array("category_id"=>$item->category_id->id, "slug"=>SiteHelper::getSlug( $item->category_id ))) ?>" title="курорты <?= $item->category_id->name ?>"><?= $item->category_id->name ?></b></a>
    </div>
</div>
