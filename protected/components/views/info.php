<div class="IBItem">
    <?php if( $item->image ) : ?>
        <div class="IBIImage">
            <a href="<?= SiteHelper::createUrl("/".$link."/description")."/".$item->slug ?>.html" title="<?= SiteHelper::getStringForTitle( $item->name ) ?>"><img src="<?= ImageHelper::getImage($item->image, 2) ?>" alt="<?= SiteHelper::getStringForTitle( $item->name ) ?>" /></a>
        </div>
    <?php endif; ?>
