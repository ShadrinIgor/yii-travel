...
<div class="IBItem">
    <div class="IBIImage">
        <a href="<?= SiteHelper::createUrl("tours/description", array( "id"=>$tour->id, "slug"=>$tour->slug )) ?>" title="<?= SiteHelper::getTranslateForUrl( $tour->name ) ?>"><img src="<?= ImageHelper::getImage($tour->image, 2) ?>" alt="<?= SiteHelper::getTranslateForUrl( $tour->name ) ?>" /></a>
    </div>
    <?php if( $tour->col>0 ) : ?><div class="floatRight rightInfo">просмотров: <b><?= $tour->col ?></b></div><?php endif; ?>
    <a href="<?= SiteHelper::createUrl("tours/description", array( "id"=>$tour->id, "slug"=>$tour->slug )) ?>" title="<?= SiteHelper::getTranslateForUrl( $tour->name ) ?>"><?= SiteHelper::getSubTextOnWorld( $tour->name, 100 ) ?></a><br/>
