<div class="IBItem">
    ...
    <div class="LParams">
        ...
        <a href="<?= SiteHelper::createUrl("hotels/index", array("country_id"=>$item->country_id->id, "slug"=>SiteHelper::getSlug( $item->country_id ))) ?>" title="отели <?= $item->country_id->name_2 ?>"><?= $item->country_id->name ?></a><br/>
        <a href="<?= SiteHelper::createUrl("hotels/index", array("city_id"=>$item->city_id->id, "slug"=>SiteHelper::getSlug( $item->city_id ))) ?>" title="отели <?= $item->city_id->name ?>"><?= $item->city_id->name ?></a>
    </div>
</div>
