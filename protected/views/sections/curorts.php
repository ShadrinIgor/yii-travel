    ...
    foreach( $items as $firmItem ):
        ...
        <tr <?= $firmItem->hot==1 ? 'class="isHot"' : "" ?>>
            ...
            <td class="textAlignJustify">
                <a href="<?= SiteHelper::createUrl("/resorts/description")."/".$firmItem->slug ?>" title="описание туристического предложения"><?= $firmItem->name ?></a><br/>
                <?= SiteHelper::getSubTextOnWorld( $firmItem->description, 300 ) ?>
                <div class="itemAction textAlignRight">
                    <a href="<?= SiteHelper::createUrl("/resorts/category")."/".$firmItem->category_id->slug ?>"><?= $firmItem->category_id->name ?></a><br/>
                    <a href="<?= SiteHelper::createUrl("/resorts/description")."/".$firmItem->slug ?>">Описание</a><br/>
                </div>
            </td>
        </tr>
    <?php endforeach; ?>
